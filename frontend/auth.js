(() => {
  ...
  document.querySelector('#authForm').onsubmit = async e => { e.preventDefault(); message(''); const payload = { username: document.querySelector('#authUsername').value, password: document.querySelector('#authPassword').value }; if (register) payload.email = document.querySelector('#authEmail').value; try { const res = await fetch(register ? '/api/auth/register' : '/api/auth/login', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(payload) }); if (!res.ok) throw new Error(await res.text()); const user = await res.json(); document.querySelector('#authButton').textContent = user.username || '已登录'; panel().classList.add('hidden'); window.dispatchEvent(new Event('auth:ready')); } catch (err) { message(err.message || '登录失败'); } };
  fetch('/api/auth/me').then(r => r.ok ? r.json() : null).then(user => { if (user) { const button=document.querySelector('#authButton'); if (user.ip_whitelisted) { button.textContent='白名单访问'; button.title='当前 IP 已在白名单中，无需登录'; } else if (user.username) { button.textContent=user.username; } } window.dispatchEvent(new Event('auth:ready')); }).catch(() => window.dispatchEvent(new Event('auth:ready')));
})();
