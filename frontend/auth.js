(() => {
  ...
  fetch('/api/auth/me').then(r => r.ok ? r.json() : null).then(user => { if (!user) return; const button=document.querySelector('#authButton'); if (user.ip_whitelisted) { button.textContent='白名单访问'; button.title='当前 IP 已在白名单中，无需登录'; } else if (user.username) { button.textContent=user.username; } }).catch(() => {});
})();
